...
- (instancetype)init {
  return [self initWithFrame:CGRectMake(0, ([UIScreen mainScreen].bounds.size.height+44-300)/2, [UIScreen mainScreen].bounds.size.width, 300 ) labelFont:[UIFont systemFontOfSize:[UIFont labelFontSize]] touchRadius:kHelpViewDefaultTouchRadius];
...
- (void)setTouchRadius:(CGFloat)touchRadius {
 ...
 // self.touchView.frame = CGRectMake(CGRectGetMinX(self.touchView.frame), CGRectGetMinY(self.touchView.frame), self.touchRadius * 2, self.touchRadius * 2);
    ...
    self.touchView.frame = CGRectMake(0, 0, 300, 300);
}
...
- (void)setUpViews {
  ...
  //self.touchView = [[AYTouchView alloc] initWithFrame:CGRectMake(0, -100, self.touchRadius * 2, self.touchRadius * 2)];
